...
public class CaptchaServiceImpl implements CaptchaService {
    ...
    private ImageVerificationVo selectOperationVerificationCode(ImageVerificationDto imageVerificationDto) throws ServiceException {
        ...
        try {
            ...
            getRequest().getSession().setAttribute("operationCode", result);
    ...
    private ImageVerificationVo selectCharVerificationCode(ImageVerificationDto imageVerificationDto) throws ServiceException {
        ...
        try {
            ...
            getRequest().getSession().setAttribute("imageVerificationVo", imageVerificationVo);
            getRequest().getSession().setAttribute("charCode", text);
            //  在分布式应用中，可将session改为redis存储
            ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
            ImageIO.write(bufferedImage, "png", byteArrayOutputStream);
            bytes = byteArrayOutputStream.toByteArray();
...
//            outputStream.flush();
    ...
    @Override
    public boolean checkCharResult(String charCode) {
        HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest();
        String code = (String) request.getSession().getAttribute("charCode");
        if (code == null) {
            return false;
        }
        return code.equals(charCode);
    }

    @Override
    public boolean checkOperationResult(String charCode) {
        HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest();
        String code = (String) request.getSession().getAttribute("operationCode");
        if (code == null) {
            return false;
        }
        return code.equals(charCode);
