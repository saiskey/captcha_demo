...
<div class="comImageValidate rightValidate">
    ...
    <div class="hkinnerWrap" style="height:60px; position: relative">
        <span class="char"  style="font-size: 12px;line-height: 30px;color: #A9A9A9;">请输入验证码</span>
        <input id="char"/>
        <span style="height:30px"><button id="submitChar">提交</button>
...
<script>
    $("#submitChar").click(function(){
        var value = $("#char").val()
        $.post("http://localhost:8080/captcha/check/operation/result",{charCode:value},function(result){
            $("#result").html(result);
        });
    });
</script>
